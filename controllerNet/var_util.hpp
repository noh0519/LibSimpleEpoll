...
std::string base64_encode(std::vector<uint8_t> &v) {
  std::string str;
...
std::string base64_decode(const std::string &s) {
  ...
  if ((leng % 4) == 0) {
    ...
    for (i = 0, ix = leng - (leng % 4); i < ix; i += 4) {
      q << (char)((base64_reverse[(unsigned char)s[i]] << 2) + ((base64_reverse[(unsigned char)s[i + 1]] & 0x30) >> 4));
      q << (char)(((base64_reverse[(unsigned char)s[i + 1]] & 0xf) << 4) + ((base64_reverse[(unsigned char)s[i + 2]] & 0x3c) >> 2));
...
std::string base64_decode(std::vector<uint8_t> &v) {
  std::string str;
  str.assign(v.begin(), v.end());
