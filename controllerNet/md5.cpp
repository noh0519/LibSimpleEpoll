...
unsigned char MD5::parse_hex(char c) {
  if ('0' <= c && c <= '9')
    return c - '0';
  if ('A' <= c && c <= 'F')
    return c - 'A' + 10;
...
/*
uint8_t *MD5::md5(const std::string str) {
  init();
  update(str.c_str(), str.length());
  finalize();
  std::string hd = hexdigest();
  uint8_t *retval = (uint8_t *)malloc(sizeof(uint8_t) * 16);
  for (int32_t i = 0; i < 16; i++) {
    retval[i] = 16 * parse_hex(hd[2 * i]) + parse_hex(hd[2 * i + 1]);
  }
...
void MD5::md5(const std::string str, uint8_t *hashval) {
  init();
  update(str.c_str(), str.length());
  finalize();
  std::string hd = hexdigest();
  for (int32_t i = 0; i < 16; i++) {
    hashval[i] = 16 * parse_hex(hd[2 * i]) + parse_hex(hd[2 * i + 1]);
  }
}

std::vector<uint8_t> MD5::md5(const std::string str) {
  init();
  update(str.c_str(), str.length());
  finalize();
  std::string hd = hexdigest();
  ...
  for (int32_t i = 0; i < 16; i++) {
    retval.push_back(16 * parse_hex(hd[2 * i]) + parse_hex(hd[2 * i + 1]));
  }

  return std::vector<uint8_t>(retval);
}

std::vector<uint8_t> MD5::hmac_md5(const std::vector<uint8_t> msg, const std::vector<uint8_t> key) {
  init();
  ...
  std::vector<uint8_t> retval;
  retval.insert(retval.end(), digest, digest + MD5::HASH_SIZE);
